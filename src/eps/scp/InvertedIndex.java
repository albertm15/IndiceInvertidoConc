...
public class InvertedIndex
{
    ...
    private ConcurrentSkipListMap<Integer,String> Files = new ConcurrentSkipListMap<Integer,String>();

    // Hash Map para acceder a las líneas de todos los ficheros del indice.
    private ConcurrentSkipListMap<Location, String> IndexFilesLines = new ConcurrentSkipListMap<Location, String>();

    // Hash Map que implementa el Índice Invertido: key=word, value=Locations(Listof(file,line)).
    private ConcurrentSkipListMap<String, ConcurrentSkipListSet <Location>> Hash =  new ConcurrentSkipListMap<String, ConcurrentSkipListSet <Location>>();
    ...
    // Getters
    public ConcurrentSkipListMap<Integer, String> getFiles() { return Files; }
    public ConcurrentSkipListMap<Location, String> getIndexFilesLines() { return IndexFilesLines; }
    public ConcurrentSkipListMap<String, ConcurrentSkipListSet<Location>> getHash() { return Hash; }
    ...
    public void buidIndex()
    {
        ...
        TotalProcessedFiles = taskDir.getTotalFiles();
        TotalLocations = taskDir.getTotalLocations();
        TotalLines = taskDir.getTotalLines();
        TotalWords = taskDir.getTotalWords();
    ...
    public void saveIndex(String indexDirectory)
    {
        ...
        SaveInvertedIndex taskInvertedIndexSave = new SaveInvertedIndex(indexDirectory, Hash);
        ...
        SaveFilesIds taskFileIdSave = new SaveFilesIds(indexDirectory, Files);
        ...
        SaveFilesLines taskFilesLinesSave = new SaveFilesLines(indexDirectory, IndexFilesLines);
        ...
        for (int i = 0; i < saveTasksThreads.size(); i++){
            try{
                saveTasksThreads.get(i).join();
            }
            catch (InterruptedException e){
                e.printStackTrace();
            }
    ...
    public void loadIndex(String indexDirectory)
    {
        ...
        LoadInvertedIndex taskInvertedIndexLoad = new LoadInvertedIndex(indexDirectory, Hash);
        ...
        LoadFilesIds taskFileIdLoad = new LoadFilesIds(indexDirectory, Files);
        ...
        LoadFilesLines taskFilesLinesLoad = new LoadFilesLines(indexDirectory, IndexFilesLines);
        ...
        for (int i = 0; i < loadTasksThreads.size(); i++){
            try{
                loadTasksThreads.get(i).join();
            }
            catch (InterruptedException e){
                e.printStackTrace();
            }
